...
. In your container image, indicate whether you prefer to use _Post Quantum Cryptography_ (in the form of _SLH-DSA_ PQC.  Default is the use of non-PQC `Schnorr` signatures ).
+
[source,bash]
-----
export USE_PQC=true
...
.. If you set _USE_PQC=false_, then the locking scripts of your TapTree leaves will utilize _Schnorr_ cryptography.
+
Schnorr is not quantum-resistant.  However, its signature size is relatively small: 64 bytes.

.. If you set _USE_PQC=true_, then the locking scripts of your TapTree leaves will utilize _SLH-DSA_ cryptography.
+
...
If using PQC, this script commits to a 32-byte SLH-DSA pub key and a OP_SUCCESS127 (represented as `OP_SUBSTR`) opcode.
